...
function placeholderItem(id: string, description: string, value: number, category: BudgetItem["category"] = "civil"): BudgetItem {
  return {
    ...
    notes: "Valor placeholder editavel; substituir por cotacao formal do sistema EPS.",
    requiresConfirmation: true,
  };
}

export function calculateMonolithicEpsBudget(context: ConstructionMethodCalculationContext): BudgetSummary {
  const { project } = context;
  const geometry = calculateMonolithicEpsGeometry(context);
  const items = calculateMonolithicEpsMaterialList(context).map(asBudgetItem);
  ...
  items.push(
    placeholderItem("eps-foundation-placeholder", "Fundacao, arranques e escoramento - placeholders", civilPlaceholderBRL),
    placeholderItem("eps-openings-placeholder", "Esquadrias, reforcos e fechamentos - placeholders", facadePlaceholderBRL),
    placeholderItem("eps-labor-equipment", "Mao de obra especializada e equipamentos - placeholders", laborEquipmentBRL, "labor"),
    placeholderItem("eps-technical-legal", "Arquitetura, engenharia, ART/RRT e aprovacao municipal", technicalLegalBRL, "technical"),
    {
      id: "eps-contingency",
...
export function calculateMonolithicEpsBudgetItems(context: ConstructionMethodCalculationContext): BudgetItem[] {
  return calculateMonolithicEpsBudget(context).items;
}
